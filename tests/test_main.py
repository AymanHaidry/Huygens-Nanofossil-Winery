...
These mock llama-cpp-python entirely since it has heavy native deps.
"""
...
class TestCleanResponse:
    def test_strips_think_tags(self):
        text = "<think hidden>reasoning</think visible>Actual answer"
...
class TestExecuteSearches:
    ...
    def test_warns_on_few_sources(self, mock_search, capsys):
        ...
        assert "Very few sources" in captured.err or "Very few sources" in captured.out
...
class TestFetchSources:
    def test_prioritizes_primary(self):
        ...
        # primary should be fetched first
        assert sources[1].fetched_text  # s2 was prioritized
...
class TestSynthesize:
    def test_full_pipeline(self, mock_llm):
        from winery.models import Source, Finding
        mock_llm.create_chat_completion.return_value = {
